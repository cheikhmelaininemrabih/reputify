          ...
          <>
            ...
            {lenders.length > 0 && (
              <section className="card pad" style={{ marginBottom: 16 }}>
                <h3 style={{ marginTop: 0 }}>Sign in as an existing lender (demo)</h3>
            ...
            <section className="card pad">
              <h3 style={{ marginTop: 0 }}>Register a lender identity</h3>
              <div style={{ display: "flex", gap: 10 }}>
                <input className="inp" placeholder="Lender / institution name" value={newName} onChange={(e) => setNewName(e.target.value)} />
                <button className="btn gold" disabled={busy || !newName} onClick={registerAndSignIn}>Register &amp; sign in</button>
              </div>
            </section>
          </>
        )}
